...
def clean_customer_id(x):
    ...
    if s.endswith('.0'):
        s = s[:-2]
    # 如果是純數字字串，轉成整數再轉字串，去除前導零或科學記號影響
    if s.isdigit():
        return str(int(s))
    return s
...
if file_a is not None and file_b is not None:
    try:
        ...
        if missing_a or missing_b:
            ...
            # 🎯 強力清理 A 檔與 B 檔的客戶編號
            df_a_filtered['客戶編號'] = df_a_filtered['客戶編號'].apply(clean_customer_id)
            df_b['客戶編號'] = df_b['客戶編號'].apply(clean_customer_id)
            
            # 2. 處理 B 檔案：移除重複資料，確保客戶編號是唯一值
