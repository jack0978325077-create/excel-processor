...
if file_a is not None and file_b is not None:
    try:
        ...
        # --- 讀取 B 檔案 (加上自動跳過空白列的聰明機制) ---
        if file_b.name.endswith('.csv'):
            df_b = pd.read_csv(file_b)
        else:
            # 嘗試正常讀取
            df_b = pd.read_excel(file_b)
            # 防呆：如果第一列抓不到客戶編號，嘗試跳過第一行再讀取
            if '客戶編號' not in df_b.columns:
                df_b = pd.read_excel(file_b, skiprows=1)
        ...
        if missing_a or missing_b:
            ...
            if missing_b: st.error(f"❌ B 檔案缺少欄位：{', '.join(missing_b)} （請確認『客戶編號』這四個字是否在表格前兩列內）")
        else:
            # 1. 處理 A 檔案：過濾豬肉類別
